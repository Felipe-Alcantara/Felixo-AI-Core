...
import { useSessionSnapshots } from '../terminal/terminal-session-context'
...
export function NotificationsPanel({
  ...
  const notifications = nodes
    ...
    .map((node) => ({ node, snapshot: snapshots[node.id] }))
    .filter(
      (notification): notification is { node: Node<CanvasNodeData>; snapshot: SessionSnapshot } =>
        isActionRequired(notification.snapshot),
    )
  ...
  return (
    <section aria-label="Notificações dos agentes" className="absolute left-4 top-16 z-30 w-96 max-w-[calc(100vw-2rem)] overflow-hidden rounded-lg border border-white/10 bg-zinc-900 shadow-2xl">
      <header className="flex items-center gap-2 border-b border-white/10 px-3 py-2 text-sm font-medium text-zinc-100">
        ...
        Notificações
        <span className="text-xs font-normal text-zinc-500">{notifications.length}</span>
      ...
      {notifications.length === 0 ? (
        ...
        <div className="max-h-[50vh] overflow-auto p-1.5">
          {notifications.map(({ node, snapshot }) => (
            <button
              key={node.id}
              type="button"
              onClick={() => {
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="mt-0.5 block text-xs text-zinc-400">{notificationText(snapshot)}</span>
              </span>
            </button>
          ))}
        </div>
