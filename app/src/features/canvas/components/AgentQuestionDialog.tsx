  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div
        ...
        className="w-full max-w-md rounded-xl border border-white/10 bg-[var(--f-surface-panel)] p-4 text-[var(--f-core-white-soft)] shadow-2xl"
      >
        ...
        <div className="flex flex-col gap-2">
          {question.opcoes.map((opcao, indice) => (
            <button
              ...
              className="felixo-btn rounded border border-white/10 bg-white/[0.04] px-3 py-2 text-left text-sm hover:border-white/30 hover:bg-white/10 disabled:opacity-50"
            >
        ...
        {error && <p className="mt-2 rounded bg-red-500/10 p-2 text-xs text-red-400">{error}</p>}
        <button
          type="button"
          disabled={busy}
