-- Adds the 'file' node type (a canvas block bound to a shared .md file).
-- SQLite can't alter a CHECK constraint in place, so the table is recreated
-- and existing rows are copied over.

CREATE TABLE canvas_nodes_new (
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL CHECK (type IN ('terminal', 'note', 'group', 'file')),
  parent_id TEXT,
