// Registra os handlers IPC do fluxo de execução/streaming de CLIs (envio de
// prompts, stop, reset e orquestração) e implementa o caminho one-shot de
// spawn. O catálogo/conta das CLIs oficiais vive em
// official-cli-account-ipc-handlers.cjs; o ciclo de vida de sessões
// persistentes vive em persistent-cli-session.cjs; helpers puros
// compartilhados vivem em cli-event-utils.cjs.
