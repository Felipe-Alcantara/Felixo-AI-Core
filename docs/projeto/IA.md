...
[2026-08-12] Bug: O prompt inicial de contexto está demorando muito para chegar no codex — segunda correção concluída. Causa: mesmo com o compositor vazio `›` do Codex visível, o store ainda exigia 500 ms de silêncio e só fazia a primeira checagem após um atraso de polling; essa espera era redundante para CLIs cuja linha de entrada já é reconhecida. Fix: a prontidão do Codex/Claude agora libera o contexto assim que a tela muda e a linha de entrada está pronta; os 500 ms continuam somente no fallback de CLIs sem detector. Teste novo cobre a entrega do contexto do Codex em até 400 ms após o compositor aparecer. Validação: Vitest frontend 401 pass, suíte Electron/Node 544 pass, build e ESLint limpos. A correção foi publicada em `v0.1.23` (Latest). Estado final: concluído; aguardando apenas conferência manual do tempo no terminal.
